...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${product.image}></img>
      <h4 class="cart-height">${product.title.slice(0, 40)}</h4>
      <p>Category: ${product.category}</p>
      <h4>Price: <span class="rating">$ ${product.price}</span></h4>
      <h4>Rating: <span class="rating">${product.rating.rate}</span></h4>
      <h5>count: <span class="rating">${product.rating.count}</span></h5>
